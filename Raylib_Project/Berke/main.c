    ...
    while (!WindowShouldClose())
    {
        ...
        case STATE_GAME:
           ...
           // --- Player 1 Skill (KEY_DOWN) ---
            ...
            // --- PLAYER 1 (Alt Taraf) FIRLATMA KONTROLÜ ---
            if (!ball1.isMoving) {
                // Topu platformun üzerinde merkeze sabitle
                ball1.position.x = player1.rect.x + player1.rect.width / 2.0f;
                ball1.position.y = player1.rect.y - ball1.radius;

                if (IsKeyPressed(KEY_UP)) {
                    ball1.speed = (Vector2){ baseBallSpeed * scaleX, -baseBallSpeed * scaleY };
                    ball1.isMoving = true;
                }
            }

            // --- PLAYER 2 (Üst Taraf) FIRLATMA KONTROLÜ ---
            if (!ball2.isMoving) {
                // Topu platformun altında merkeze sabitle
                ball2.position.x = player2.rect.x + player2.rect.width / 2.0f;
                ball2.position.y = player2.rect.y + player2.rect.height + ball2.radius;

                // Eğer Bot moduysa otomatik fırlat, değilse 'W' tuşunu bekle
                if (isBotMode) {
                    // Küçük bir şansla veya kısa bir süre sonra botun topu fırlatmasını sağlayabilirsin
                    if (rand() % 100 < 5) { // Her karede %5 ihtimalle fırlatır (hızlıca başlar)
                        ball2.speed = (Vector2){ -baseBallSpeed * scaleX, baseBallSpeed * scaleY };
                        ball2.isMoving = true;
                    }
                }
                else {
            ...
            if (!ball1.isMoving) {
                // Top fırlatılmadıysa: Platformun üzerinde onu takip etsin
                ball1.position.x = player1.rect.x + player1.rect.width / 2.0f;
                ball1.position.y = player1.rect.y - ball1.radius;

                // Yukarı ok tuşuna basıldığında fırlat
                if (IsKeyPressed(KEY_UP)) {
                    ball1.speed = (Vector2){ baseBallSpeed * scaleX, -baseBallSpeed * scaleY };
                    ball1.isMoving = true;
                }
                ...
                // Top fırlatıldıysa: Senin gönderdiğin hareket ve çarpışma kodları çalışsın
            ...
            if (!ball2.isMoving) {
                // Top fırlatılmadıysa: Platformun altında onu takip etsin
                ball2.position.x = player2.rect.x + player2.rect.width / 2.0f;
                ball2.position.y = player2.rect.y + player2.rect.height + ball2.radius;

                // Bot modundaysa otomatik, değilse 'W' tuşu ile fırlat
