    ...
    while (!WindowShouldClose())
    {
        ...
        case STATE_GAME:
            ...
            case STATE_PAUSE:
                ...
                DrawText("OYUN DURAKLATILDI", screenWidth / 2 - MeasureText("OYUN DURAKLATILDI", 30 * scaleY) / 2, screenHeight * 0.35f, 30 * scaleY, RAYWHITE);

                Rectangle resRect = { screenWidth / 2 - 100 * scaleX, screenHeight * 0.5f, 200 * scaleX, 40 * scaleY };
                Rectangle menRect = { screenWidth / 2 - 100 * scaleX, screenHeight * 0.6f, 200 * scaleX, 40 * scaleY };
                ...
                // Görsel Seçenekler
                DrawText("Devam Et", resRect.x + 20 * scaleX, resRect.y, 25 * scaleY, (pauseSelection == 1) ? YELLOW : LIGHTGRAY);
                DrawText("Menuye Don", menRect.x + 20 * scaleX, menRect.y, 25 * scaleY, (pauseSelection == 2) ? YELLOW : LIGHTGRAY);
                ...
                if (IsKeyPressed(KEY_ENTER) || (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && CheckCollisionPointRec(GetMousePosition(), (pauseSelection == 1 ? resRect : menRect)))) {
                    if (pauseSelection == 1) currentScreen = STATE_GAME;
                        ...
                        // Menüye dönerken skoru kaydet (Opsiyonel)
