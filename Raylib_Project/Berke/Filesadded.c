...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if (currentScreen == STATE_MENU) {
            DrawText("PONG BREAKER ROYALE OYNA", screenWidth * 0.31f, screenHeight * 0.44f, screenHeight * 0.044f, BLACK);
...
void LoadScores() {
    FILE* file = fopen("scores.txt", "rb"); //Fonksiyonun icindekiler:dosya adi(veya path) ile dosya modu(modes)
    if (file == NULL) {
        ...
        fread(topScores, sizeof(ScoreEntry), MAX_SCORES, file); //fread ile skorlari dosyadan yukleme (Load).
        fclose(file);
    }
}

void SaveScore(const char* name, int newScore) {
    // 1. Yeni skoru listenin sonuna uygunsa yerleştir (Basit bir sıralama algoritması)
    for (int i = 0; i < MAX_SCORES; i++) {
        if (newScore > topScores[i].score) {
            // Aşağıdaki skorları kaydır
            for (int j = MAX_SCORES - 1; j > i; j--) {
                topScores[j] = topScores[j - 1];
            }
            // Yeni skoru yerleştir
    ...
    // 2. Güncel listeyi dosyaya yaz
    FILE* file = fopen("scores.txt", "wb"); //wb write in binary (mode)
    if (file != NULL) {
        fwrite(topScores, sizeof(ScoreEntry), MAX_SCORES, file);    //fwrite ile dosyaya yazariz (filewrite)
        fclose(file);
    }
}

void DrawLeaderboard() {
    DrawText("EN YUKSEK SKORLAR", 300, 100, 20, GOLD);

    for (int i = 0; i < MAX_SCORES; i++) {
        DrawText(TextFormat("%d. %s: %d", i + 1, topScores[i].name, topScores[i].score),
            300, 140 + (i * 30), 20, LIGHTGRAY);
    }
}